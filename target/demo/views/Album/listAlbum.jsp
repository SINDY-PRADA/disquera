...
<table>
 ...
 <tbody>
     <c:forEach var="albumList" items="${listar}">
     <tr>
      <td>${albumList.getIdAlbum()}</td>
      <td>${albumList.getNombreAlbum()}</td>
      <td>${albumList.getAnioPublicacion()}</td>

         <td>
         <c:if test="${albumList.getEstadoAlbum() == true}">
         <span>Activo</span>
         </c:if>
         <c:if test="${albumList.getEstadoAlbum() == false}">
             <span>Inactivo</span>
         </c:if>
         </td>
         <td>
             <c:if test="${albumList.getEstadoAlbum() == false}">
             <a href="album?accion=estado&idAlbum=${albumList.getIdAlbum()}&estadoAlbum=true"><button class="btn btn-primary" ><span>Activar</span></button></a>
         </c:if>
             <c:if test="${albumList.getEstadoAlbum() == true}">
             <a href="album?accion=estado&idAlbum=${albumList.getIdAlbum()}&estadoAlbum=false"><button class="btn btn-danger"><span>Inactivar</span></button></a>
         </c:if>
         </td>
         <td>
            <div>
               <a href="album?accion=eliminarAlbum&idAlbum=${albumList.getIdAlbum()}"><img src="assets/img/eliminar.png" style="width: 25px;"></a>
               <a href="album?accion=editarAlbum&idAlbum=${albumList.getIdAlbum()}"><button type="button" class="btn btn-info">Editar</button></a>
...
<div>
   ...
   <br>
   <br>
<a href="album?accion=añadirAlbum"><button class="btn btn-dark">Agregar Álbum</button></a>
<br>
<br>
<br>
<br>
